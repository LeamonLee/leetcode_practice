...
class Solution:
    def rob(self, nums: List[int]) -> int:
        ...
        def helper(startIdx, endIdx):
            if endIdx - startIdx == 0: return nums[startIdx]
            if endIdx - startIdx == 1: return max(nums[startIdx], nums[endIdx])
            
            dp = [0]*(endIdx - startIdx + 1)
            dp[0] = nums[startIdx]
            dp[1] = max(nums[startIdx], nums[startIdx+1])

            for i in range(2, len(dp)):
                dp[i] = max(nums[startIdx+i]+dp[i-2], dp[i-1])
