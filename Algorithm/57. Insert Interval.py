...
class Solution:
    def insert(self, intervals: List[List[int]], newInterval: List[int]) -> List[List[int]]:
        res=[]
        isAdded = False # 紀錄newInterval是否已經被insert

        '''
        這題題目有說已經先幫你根據每個element的start排序好了，且都未重疊
        '''

        for i in range(len(intervals)):
            # 這是一個技巧，取各自第一個item的最大值，和各自第二個item的最小值互相比較
            # 如果第一個item最大值<=第二個item最小值代表有重疊
            ...
            if start <= end:    # 代表有重疊
                # 讓兩個interval merge起來，並更新newInterval
                newInterval[0] = min(newInterval[0], intervals[i][0])
                newInterval[1] = max(newInterval[1], intervals[i][1])
            else:   # 沒重疊
                # 確認newInterval的end是否比當前interval的start還小，如果比較小且還未被加入，就加進res，否則和下一個interval比
